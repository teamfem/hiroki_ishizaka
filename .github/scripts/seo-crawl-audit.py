...
# Exclude Git internals / generated dependency directories from page audit.
...
for p in html_files:
    ...
    if u in sitemap_set:
        ...
        if p.name!='404.html' and 'noindex' not in robots:
    ...
    for href in re.findall(r'<a\b[^>]*\bhref=["\']([^"\']+)["\']',html,re.I):
        ...
        if parsed.scheme in ('http','https'):
            ...
            # resolve relative to current public URL
            absu=urljoin(u,href)
            if not absu.startswith(ROOT): continue
            target=url_to_file(clean_url(absu))
        if target is None: continue
        # URL without trailing slash can still correspond to an HTML file or directory index.
        candidates=[target]
        if target.suffix=='':
